...
public class Util {
    ...
    public static void main(String[] args) {
        ...
        Monster.addWeapons(builder, weapons_add);
...
//        Monster.finishMonsterBuffer(builder, orc);
        ...
        String firstWeaponName = monster.weapons(0).name();
        short firstWeaponDamage = monster.weapons(0).damage();

        String secondWeaponName = monster.weapons(1).name();
        short secondWeaponDamage = monster.weapons(1).damage();

        System.out.println("weaponsLength: "+weaponsLength+"  firstWeaponName: "+firstWeaponName+"  firstWeaponDamage: "+firstWeaponDamage+"  secondWeaponName: "+secondWeaponName+"  secondWeaponDamage: "+secondWeaponDamage);
        int unionType = monster.equippedType();
        if (unionType == Equipment.Weapon) {
            Weapon weapon = (Weapon)monster.equipped(new Weapon()); // Requires explicit cast
            // to `Weapon`.
            String equippedWeaponName = weapon.name();    // "Axe"
            short equippedWeaponDamage = weapon.damage(); // 5
            System.out.println("equippedWeaponName: "+equippedWeaponName+"  equippedWeaponDamage: "+equippedWeaponDamage);
        }
